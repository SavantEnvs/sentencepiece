...
extern "C" int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size) {
  ...
  if (ops & 0x10) {
    // Encode as SentencePieceText proto (replaces the removed *AsSerializedProto helpers;
    // exercises the same protobuf-producing encode path).
    sentencepiece::SentencePieceText spt;
    g_processor->Encode(text, &spt);

    sentencepiece::NBestSentencePieceText nbest_spt;
    g_processor->NBestEncode(text, 2, &nbest_spt);

    sentencepiece::SentencePieceText sample_spt;
    g_processor->SampleEncode(text, -1, 0.5, &sample_spt);
  }
  ...
  // Entropy calculation (CalculateEntropy) was removed from the public API upstream; the
  // ops & 0x80 fuzz surface it used to drive no longer exists.
